"""Run CMA-ES with 50K evaluations and 8-core parallelism."""
...
def evaluate_batch(solutions):
    """Evaluate entire population batch on GPU at once."""
    synth = SynthPatch()
    ...
    for params_np in solutions:
        ...
        total = 0.0
        for audio_np, freq in TARGET_DATA:
            target = torch.tensor(audio_np, dtype=torch.float32, device=DEVICE).unsqueeze(0)
            dur = len(audio_np) / 44100
            gen = synth.render(params.cpu(), f0_hz=freq, duration=dur, note_duration=dur * 0.9)
            gen = gen.to(DEVICE)
...
def evaluate_single(params_np):
    """Fallback single eval for multiprocessing."""
    synth = SynthPatch()
    loss_fn = get_loss("matching")
    ...
    for audio_np, freq in TARGET_DATA:
        ...
        loss = loss_fn(gen[:, :ml].unsqueeze(0), target[:, :ml].unsqueeze(0))
        total += loss.item()
    return total / len(TARGET_DATA)


if __name__ == "__main__":
    # Spectral init
    init_15 = spectral_init(TARGET_DATA[2][0], 44100)
    init_18 = torch.cat([init_15, torch.tensor([0.5, 0.3, 0.3])]).numpy()

    BUDGET = 500000
    POPSIZE = 80  # larger population for more exploration

    print(f"=== CMA-ES: {BUDGET} evals, 18 params, 8-core parallel ===")
    print(f"Budget: {BUDGET} evals (Hansen recommends {100*18**2}-{1000*18**2} for n={18})")

    t0 = time.time()

    es = cma.CMAEvolutionStrategy(init_18, 0.5, {  # larger sigma for more exploration
        'bounds': [[0]*18, [1]*18],
        'maxfevals': BUDGET,
        'popsize': POPSIZE,
        'verbose': -9,
        'seed': 42,
        'tolx': 1e-8,       # tighter tolerance — don't stop early
        'tolfun': 1e-10,     # tighter tolerance
        'tolstagnation': 50000,  # allow long stagnation before giving up
    })
    ...
    # Save params
    np.save('output/best_params_v11.npy', best_params.numpy())
    ...
    sf.write('output/matched_v11_50k.wav', full_audio.astype(np.float32), 44100)

    # Diagnostic
    match_a = full_audio[s:e]
    orig_a = orig_full[s:e]
    cent_o = np.mean(librosa.feature.spectral_centroid(y=orig_a, sr=44100))
    cent_m = np.mean(librosa.feature.spectral_centroid(y=match_a, sr=44100))
    print(f"\nCentroid: orig={cent_o:.0f} match={cent_m:.0f} diff={abs(cent_o-cent_m):.0f} Hz")

    print("\n--- ORIGINAL ---")
    subprocess.run(["afplay", "target.wav"])
    print("--- V11 (50K evals, 8-core parallel) ---")
