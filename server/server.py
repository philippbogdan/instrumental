...
# The frontend is part of the personal-website repo now (its deployed home);
# this only matters for serving the app locally in development. The backend
# repo keeps its own root for events and fitted params.
ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
APP_DIR = os.environ.get(
    "INSTRUMENTAL_APP_DIR",
    os.path.expanduser("~/personal-website/public/instrumental"))
...
# Analytics
# ---------------------------------------------------------------------------
EVENTS_FILE = os.path.join(ROOT, "events.jsonl")
...
async def preset_best():
    ...
    params_path = os.path.join(ROOT, "output", "best_params_v24.npy")
...
async def stats_page():
    """Local analytics page; lives with the backend, not the public app."""
    return FileResponse(os.path.join(os.path.dirname(os.path.abspath(__file__)), "stats.html"))
